...
def reset():
	...
	for k in _total_times.keys():
		if k not in _all_times.keys():
			_all_times[k] = []
		_all_times[k].append(_total_times[k])
	_total_times.clear()
	_start_times.clear()
	_timer_stack.clear()
	_running_timer = None
...
def print_all_stats(start=500):
        """ Prints the current timing information into a table. """
        print()
        #print(_all_times)
        _all_times_2 = {k:_all_times[k][start:] for k in _all_times.keys()}
        all_fn_names = [k for k in _all_times_2.keys() if k not in _disabled_names]
        print(all_fn_names)
        max_name_width = max([len(k) for k in all_fn_names] + [4])
        if max_name_width % 2 == 1: max_name_width += 1
        format_str = ' {:>%d} | {:>10.4f} +- {:>10.4f} ' % max_name_width
        ...
        for name in all_fn_names:
                print(format_str.format(name, np.mean(_all_times_2[name])*1000, np.std(_all_times_2[name])*1000))

        print(sep_text)
        all_total_times = [np.sum([_all_times_2[k][i] for k in _all_times_2.keys() if k not in _disabled_names]) for i in range(len(_all_times_2[name]))]
        print(format_str.format('Total', np.mean(all_total_times)*1000, np.std(all_total_times)*1000))
        print()
