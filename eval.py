...
def parse_args(argv=None):
    ...
    parser.add_argument('--display_polygons', default=True, type=str2bool,
                        help='Whether or not to display convex polygons around masks')
    ...
    parser.add_argument('--bbox_det_file', default='/bbox_detections.json', type=str,
                        help='The output file for coco bbox results if --coco_results is set.')
    parser.add_argument('--mask_det_file', default='/mask_detections.json', type=str,
                        help='The output file for coco mask results if --coco_results is set.')
    parser.add_argument('--poly_det_file', default='/poly_detections.json', type=str,
                        help='The output file for coco poly results if --coco_results is set.')
    parser.add_argument('--convex_det_file', default='/convex_detections.json', type=str,
                        help='The output file for coco convex poly results if --coco_results is set.')
    ...
    parser.add_argument('--score_fold', default='mAP/')
    parser.add_argument('--convex_poly', default=False, type=str2bool,
                         help='Turn predicted convex polygon to a verified convex polygon.')
    parser.add_argument('--suffix', default='', type=str)

    parser.set_defaults(no_bar=False, display=False, resume=False, output_coco_json=False, shuffle=False,
                        benchmark=False, no_sort=False, no_hash=False, mask_proto_debug=False, crop=True, detect=False, display_fps=False,
                        emulate_playback=False,prediction=False)
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Copy'):
        ...
        if cfg.eval_mask_branch:
            ...
            masks = t[4][idx]
        classes, scores, center_scores, boxes = [x[idx].cpu().numpy() for x in t[:4]]
    ...
    if args.display_polygons:
        polygons = list(t[-1].cpu().numpy().astype(np.int32))
        for j,polygon in enumerate(polygons):
            cv2.drawContours(img_numpy,[polygon],0,get_color(j),2)
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            if args.display_text:
                ...
                cv2.rectangle(img_numpy, (x1, y2), (x1 + text_w, y2 - text_h - 4), color, -1)
...
def prep_benchmark(dets_out, h, w):
    ...
    with timer.env('Copy'):
        ...
        if isinstance(scores, list):
            ...
            poly_scores = scores[2].cpu().numpy()
        else:
            scores = scores.cpu().numpy()
        classes = classes.cpu().numpy()
        boxes = boxes.cpu().numpy()
        masks = masks.cpu().numpy()
        polygons = poly_coords.cpu().numpy()
...
class Detections:
    ...
    def add_bbox(self, image_id:int, category_id:int, bbox:list, score:float, centerness:float):
        ...
        self.bbox_data.append({
            ...
            'bbox': bbox,
            'score': float(score),
            'centerness': float(centerness),
        })

    def add_mask(self, id:int, image_id:int, category_id:int, bbox:list, segmentation:np.ndarray, score:float, centerness:float):
    ...
    def add_poly(self, id:int, image_id:int, category_id:int, bbox:list, poly:np.ndarray, score:float, centerness:float):
        ...
        poly = [round(float(x)*10)/10 for x in poly.ravel().tolist()]
    ...
    def dump(self):
        dump_arguments = [
            ...
            (self.poly_data, args.poly_det_file),
        ]
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, gt_polygons, h, w, num_crowd, image_id, detections:Detections=None):
    ...
    if not args.output_coco_json:
        with timer.env('Prepare gt'):
            ...
            gt_polygons = torch.Tensor(gt_polygons)
            
            indices_gt_pol = gt_polygons[:,:,0] !=-1
            
            gt_poly_masks = poly2mask(torch.ones_like(torch.Tensor(gt_masks)).moveaxis(0,2),
                                      gt_polygons.clone().flip(1).moveaxis(1,2), 
                                      indices_gt_pol.flip(1),min_value=0).moveaxis(2,0)

            
            """
            for i in range(gt_convex_masks.shape[0]):
                print(cv2.imwrite('/home/2017018/rconda01/{}_gt_mask.png'.format(i),(gt_masks[i]*255).astype(np.uint8)))
                print(cv2.imwrite('/home/2017018/rconda01/{}_gt.png'.format(i),(gt_convex_masks[i].cpu().detach().numpy()*255).astype(np.uint8)))
            exit()
            """
            gt_masks = torch.Tensor(gt_masks).view(-1, h*w)
            gt_poly_masks = gt_poly_masks.view(-1, h*w)
            
            if num_crowd > 0:
                split = lambda x: (x[-num_crowd:], x[:-num_crowd])
                crowd_boxes  , gt_boxes   = split(gt_boxes)
                crowd_masks  , gt_masks   = split(gt_masks)
                crowd_poly_masks, gt_poly_masks = split(gt_poly_masks)
    ...
    with timer.env('Postprocess'):
        ...
        for i,bb in enumerate([boxes]):
            for j in range(bb.shape[0]):
                np.savetxt(path+'boxes/{}_{}.txt'.format(i,j),bb[j].cpu().numpy())


        os.makedirs(path+'polygons/',exist_ok=True)
        for i,pol in enumerate([poly_coords]):
            for j in range(pol.shape[0]):
                np.savetxt(path+'polygons/{}_{}.txt'.format(i,j),pol[j].cpu().numpy())

        os.makedirs(path+'masks/',exist_ok=True)
        for i,mask in enumerate([masks]):
            for j in range(mask.shape[0]):
                cv2.imwrite(path+'masks/{}_{}.png'.format(i,j),mask[j,...].cpu().numpy())
        ...
        if isinstance(scores, list):
            ...
            poly_scores = list(scores[2].cpu().numpy().astype(float))
        else:
        ...
        center_scores = list(center_scores.cpu().numpy().astype(float))
        masks = masks.reshape(-1, h*w).cuda()
        poly_masks = poly_masks.reshape(-1, h*w).cuda()
        boxes = boxes.cuda()
        poly_coords = poly_coords.cuda()
        ...
        with timer.env('JSON Output'):
            ...
            for i in range(masks.shape[0]):
                ...
                if (boxes[i, 3] - boxes[i, 1]) * (boxes[i, 2] - boxes[i, 0]) > 0:
                    detections.add_bbox(image_id, classes[i], boxes[i,:],   box_scores[i], center_scores[i])
                    detections.add_mask(id,image_id, classes[i], boxes[i,:], masks[i,:,:], mask_scores[i], center_scores[i])
                    detections.add_poly(id,image_id, classes[i], boxes[i,:], poly_coords[i,:,:], poly_scores[i], center_scores[i])
                    id+=1
            return
    ...
    with timer.env('Eval Setup'):
        ...
        poly_iou_cache = _mask_iou(poly_masks, gt_poly_masks)
        ...
        if num_crowd > 0:
            ...
            crowd_poly_mask_iou_cache = _mask_iou(poly_masks, crowd_poly_masks, iscrowd=True)
        else:
            crowd_mask_iou_cache = None
            crowd_bbox_iou_cache = None
            crowd_poly_mask_iou_cache = None

        box_indices = sorted(range(num_pred), key=lambda i: -box_scores[i])
        mask_indices = sorted(box_indices, key=lambda i: -mask_scores[i])
        poly_mask_indices = sorted(box_indices, key=lambda i: -poly_scores[i])
        ...
        iou_types = [
            ...
            ('poly', lambda i,j: poly_iou_cache[i, j].item(),
                     lambda i,j: crowd_poly_mask_iou_cache[i,j].item(),
                     lambda i: poly_scores[i], poly_mask_indices)
        ]
...
def evalimage(net:ConvexMask, path:str, save_path:str=None,detections:Detections=None):
    ...
    if detections is not None:
        ...
        for i in range(masks.shape[0]):
            # Make sure that the bounding box actually makes sense and a mask was produced
            if (boxes[i, 3] - boxes[i, 1]) * (boxes[i, 2] - boxes[i, 0]) > 0:
                detections.add_bbox(image_id, classes[i], boxes[i,:], scores[i], center_scores[i])
                detections.add_mask(id,image_id, classes[i], boxes[i,:], masks[i,:,:], scores[i], center_scores[i])
                detections.add_poly(id,image_id, classes[i], boxes[i,:], poly_coords[i,:,:], scores[i], center_scores[i])
                id+=1
    else:
...
def evalimages(net:ConvexMask, input_folder:str, output_folder:str):
    ...
    for p in Path(input_folder).glob('*'): 
        ...
        evalimage(net, path, out_path, detections)
        print(path + ' -> ' + out_path)
...
def evalvideo(net:ConvexMask, path:str, out_path:str=None):
...
def evaluate(net:ConvexMask, dataset, score_fold=None, train_mode=False):
    ...
    frame_times = MovingAverage(5000)
    ...
    if not args.display and not args.benchmark:
        ...
        ap_data = {
            ...
            'poly': [[APDataObject() for _ in cfg.dataset.class_names] for _ in iou_thresholds],
        }
        detections = Detections()
    else:
    ...
    try:
        ...
        for it, image_idx in enumerate(dataset_indices):
            ...
            with timer.env('Load Data'):
                if args.benchmark:
                    img = torch.from_numpy(dataset.pull_image(image_idx)).cuda().float()
                    h,w,_ = img.shape
                    batch = FastBaseTransform()(img.unsqueeze(0))
                else:
                    img, gt, gt_masks, gt_polygons, h, w, num_crowd, img_id = dataset.pull_item(image_idx)
                    """
                    path = '/home/2017018/rconda01/visu/'
                    os.makedirs(path,exist_ok=True)
                    os.makedirs(path+'images/',exist_ok=True)
                    ...
                    for i,bb in enumerate([gt]):
                        for j in range(bb.shape[0]):
                            np.savetxt(path+'boxes/{}_{}.txt'.format(i,j),bb[j,:-1])


                    os.makedirs(path+'polygons/',exist_ok=True)
                    for i,pol in enumerate([gt_polygons]):
                        for j in range(pol.shape[0]):
                            np.savetxt(path+'polygons/{}_{}.txt'.format(i,j),pol[j])

                    os.makedirs(path+'masks/',exist_ok=True)
                    for i,mask in enumerate([gt_masks]):
                        for j in range(mask.shape[0]):
                            cv2.imwrite(path+'masks/{}_{}.png'.format(i,j),mask[j,...])
            ...
            if args.display:
                ...
                prep_metrics(ap_data, preds, img, gt, gt_masks, gt_polygons, h, w, num_crowd, dataset.ids[image_idx], detections)
            ...
            if args.display:
                ...
                if it > 450: fps = 1 / frame_times.get_avg()
                else: fps = 0
        ...
        if not args.display and not args.benchmark:
            ...
            if args.output_coco_json:
                ...
                return calc_map(ap_data,score_fold)
        elif args.benchmark:
            print()
            print()
            print(cfg.name)
...
def calc_map(ap_data,score_fold):
    print('Calculating mAP...')
    aps = [{'box': [], 'poly': [], 'mask': []} for _ in iou_thresholds]

    for _class in range(len(cfg.dataset.class_names)):
        for iou_idx in range(len(iou_thresholds)):
            for iou_type in ('box', 'poly', 'mask'):
                ap_obj = ap_data[iou_type][iou_idx][_class]

                if not ap_obj.is_empty():
                    aps[iou_idx][iou_type].append(ap_obj.get_ap())

    all_maps = {'box': OrderedDict(), 'poly': OrderedDict(), 'mask': OrderedDict()}
    ...
    if score_fold is not None:
        with open(score_fold+'.txt','w') as txt:
...
if __name__ == '__main__':
    ...
    #os.makedirs(args.score_fold, exist_ok=True)

    if args.output_coco_json:
        os.makedirs(args.home_dir+'detections/'+cfg.name,exist_ok=True)
        args.bbox_det_file = args.home_dir+'detections/'+cfg.name+args.bbox_det_file+args.suffix
        args.mask_det_file = args.home_dir+'detections/'+cfg.name+args.mask_det_file+args.suffix
        args.poly_det_file = args.home_dir+'detections/'+cfg.name+args.poly_det_file+args.suffix
    ...
    with torch.no_grad():
        ...
        if args.cuda:
            cudnn.fastest = True
            torch.set_default_tensor_type('torch.cuda.FloatTensor') #HalfTensor')
        else:
        ...
        if args.image is None and args.video is None and args.images is None:
            if args.prediction:
                dataset = COCODetection(cfg.dataset.test_images, cfg.dataset.test_info,
                                        transform=BaseTransform(), has_gt=True) #False)
            else:
                dataset = COCODetection(cfg.dataset.valid_images, cfg.dataset.valid_info,
                                        transform=BaseTransform(resize_gt=False), has_gt=cfg.dataset.has_gt)
            #prep_coco_cats()
        else:
            dataset = None        
        ...
        net = ConvexMask()
        net.load_weights(args.save_folder+'/best_checkpoint.pth')
        net.eval()
        print(' Done.')
        ...
        evaluate(net, dataset, args.score_fold)
